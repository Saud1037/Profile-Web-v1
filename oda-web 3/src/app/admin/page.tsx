...
import { Eye, Lock, Save, ArrowRight, Check, Upload, X, AlertCircle, Loader2 } from "lucide-react";
import Link from "next/link";
import Image from "next/image";

export default function AdminPage() {
  const { profile, updateProfile, saveProfile, isLoading, isSaving } = useProfile();
  ...
  const handleSave = useCallback(async () => {
    setSaveState("idle");
    setSaveError("");
    // sync text fields into profile state first, then save
    updateProfile({ name: localName, bio: localBio, siteName: localSiteName });
    // small tick to let state update propagate
    ...
    if (result.ok) {
      ...
      setSaveState("error");
      setSaveError(result.error ?? "فشل الحفظ");
    }
  }, [localName, localBio, localSiteName, updateProfile, saveProfile]);
  ...
  /* ── Dashboard ── */
  const saveLabel = isSaving ? "جاري الحفظ..." : saveState === "ok" ? "تم الحفظ ✓" : "حفظ";
  const saveBg    = isSaving ? "#6366f180" : saveState === "ok" ? "#10b981" : saveState === "error" ? "#ef4444" : profile.colors.primary;
  ...
  return (
    <div className="min-h-screen" style={{ background: profile.colors.background, color: profile.colors.text }}>
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 backdrop-blur-md"
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} disabled={isSaving}
              ...
              style={{ background: saveBg, color: "white" }}>
              {isSaving ? <Loader2 size={13} className="animate-spin" /> : saveState === "ok" ? <Check size={13} /> : <Save size={13} />}
              {saveLabel}
            </button>
          </div>
        </div>
        {/* خطأ الحفظ */}
        {saveState === "error" && (
          <div className="max-w-2xl mx-auto px-4 pb-3 flex items-center gap-2 text-red-400 text-xs">
            <AlertCircle size={13} />
            {saveError} — تأكد من إعداد GITHUB_TOKEN في Vercel
          </div>
        )}
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        <section className="glass-card p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs opacity-30">PNG أو ICO — 32×32 أو 64×64</p>
          </div>
        </section>
        ...
        <section className="glass-card p-5 flex flex-col gap-4">
          ...
          <PasswordChange onSave={(p) => updateProfile({ adminPassword: p })} />
        </section>

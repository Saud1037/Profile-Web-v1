...
export function AdminGeneral({ profile, showToast, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (res.ok) { showToast('Profile saved ✓'); onSaved() }
      else {
    ...
    <div>
      ...
      <SectionCard title="Profile Images">
        ...
        <div className="mb-6 rounded-xl overflow-hidden border border-[var(--border)] bg-[var(--bg-secondary)] w-full max-w-xs">
          ...
          <div className="px-4 pb-4">
            ...
            <div className="font-semibold text-sm text-[var(--text)]">{form.name || 'Name'}</div>
            <div className="font-mono text-xs text-[var(--text-3)]">@{form.username || 'username'}</div>
          </div>
        </div>
        ...
        <ImageUpload
          ...
          hint="Square image recommended — shown as your profile picture"
        />
        <ImageUpload
          label="Banner"
          ...
          hint="Wide banner image — shown behind your avatar"
        />
        ...
        <Field label="Banner Color" hint="Used when no banner image is set">
          <div className="flex items-center gap-3">
            ...
            <Input value={form.banner_color} onChange={e => set('banner_color', e.target.value)} className="w-36" placeholder="#0d1f3c" />
          </div>
        </Field>
      </SectionCard>
      ...
      <SectionCard title="Personal Info">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Username" hint="Shown as @username">
            <div className="flex items-center">
              ...
              <Input value={form.username} onChange={e => set('username', e.target.value)} placeholder="saud.dev" className="rounded-l-none" />
            </div>
          </Field>
        </div>
        ...
        <Field label="Available for Work">
          <div className="flex items-center gap-3">
            <button onClick={() => set('available_for_work', !form.available_for_work)}
      ...
      <SectionCard title="Tags & Stats">
        ...
        <Field label="Stats" hint="Format: value:label — e.g. 3+:Years Exp., 20+:Projects">
          <Input value={form.stats} onChange={e => set('stats', e.target.value)} placeholder="3+:Years Exp., 20+:Projects" />
        </Field>
      </SectionCard>
